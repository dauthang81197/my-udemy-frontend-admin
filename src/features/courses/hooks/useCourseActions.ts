...
export function useUpdateCourse() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Parameters<typeof courseApi.update>[1] }) =>
      courseApi.update(id, data),
    onMutate: async ({ id, data }) => {
      await qc.cancelQueries({ queryKey: [COURSES_QUERY_KEY] });
      const snapshots = qc.getQueriesData<PaginatedResponse<Course>>({ queryKey: [COURSES_QUERY_KEY] });
      snapshots.forEach(([key, old]) => {
        if (!old) return;
        qc.setQueryData<PaginatedResponse<Course>>(key, {
          ...old,
